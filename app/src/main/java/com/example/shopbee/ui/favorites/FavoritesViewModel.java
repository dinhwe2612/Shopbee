...
public class FavoritesViewModel extends BaseViewModel<FavoritesNavigator> {
    ...
    MutableLiveData<List<OrderDetailResponse>> favoriteProducts = new MutableLiveData<>();

    public MutableLiveData<List<OrderDetailResponse>> getFavoriteProducts() {
        return favoriteProducts;
    }

    public void setFavoriteProducts(MutableLiveData<List<OrderDetailResponse>> favoriteProducts) {
...
//        );
//    }
    public void syncFavoriteProducts() {
        setIsLoading(true);
        getCompositeDisposable().add(getRepository().getUserVariation(Repository.UserVariation.FAVORITE)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
